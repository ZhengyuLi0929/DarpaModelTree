...
if d == 1:
    ...
    for option in ['EventCount', 'UserCount', 'NewUserCount']:
        ...
        for index in range(len(selected_na)):
            # read dnn results
            with open("youtube_UIUC_HYBRID_TEXT_.json") as f:
            ...
            if option == "EventCount":
                ...
                Y_pred = dnn_data[key].UserCount.tolist()
            else:
                Y_pred = dnn_data[key].NewUserCount.tolist()
            #regression = LinearRegression(fit_intercept=False,normalize=True)
            #decay(X_train, 0.85)
            ...
            for i in range(bag):
                ...
                ratio1 = sum(pred1) / (sum(Y_pred) + 0.1)
                pred = postprocess(ratio1 * np.array(Y_pred))
...
with open('youtube_UIUC_HYBRID_GDELT_.json', 'w') as outfile:
