...
def corr_to_csv():
	...
	df_twitter.to_csv('./data_csv/corr_bert_131_twitter.csv')
	df_youtube.to_csv('./data_csv/corr_bert_131_youtube.csv')
...
def generate_training_data(platform, asp, K=10):
	...
	df_event = pd.read_csv('./data_csv/'+platform+'_131_'+asp+'.csv', header=0,index_col=0)
	...
	# read correlation
	df_corr = pd.read_csv('./data_csv/corr_bert_131_'+platform+'.csv', header=0,index_col=0)

	infoIDs = sorted(df_corr.index.values) # narrative
	...
	fx_train = open(os.path.join(path, 'bert_'+asp+'_131_x.csv'), 'w')
	fi_train = open(os.path.join(path, 'bert_'+asp+'_131_ind.csv'), 'w')
	fy_train = open(os.path.join(path, 'bert_'+asp+'_131_y.csv'), 'w')
	...
	for infoID in infoIDs:
		...
		id = infoID.replace('/','#')
		fx = open('./test/'+platform+'/'+id+'_bert_'+asp+'_131_x.csv', 'w')
		fi = open('./test/'+platform+'/'+id+'_bert_'+asp+'_131_ind.csv', 'w')
		fy = open('./test/'+platform+'/'+id+'_bert_'+asp+'_131_y.csv', 'w')
		fp = open('./test/'+platform+'/'+id+'_bert_'+asp+'_131_y_prev.csv', 'w')
